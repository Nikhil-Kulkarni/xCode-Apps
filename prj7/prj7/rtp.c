...
static PACKET* packetize(char *buffer, int length, int *count){
	 ...
	 *  array.
	 */
	 PACKET* packetArr = calloc(length, sizeof(PACKET));

	 // TODO: Account for case when length is not a multiple of MAX_PAYLOAD_LENGTH
	 // Length of packetArr
	 int packetArrLen = length / MAX_PAYLOAD_LENGTH;
	 *count = packetArrLen;
	 ...
	 for (int i = 0; i < length; i++) {
		 ...
		 if (currPosInPayload == MAX_PAYLOAD_LENGTH - 1) {
